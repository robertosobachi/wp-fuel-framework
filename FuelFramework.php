<?php
/** Loading Fuel Framework ## Amazing!! ##
  ================================================== */
define('DS', '/');
$template_directory = get_template_directory();
define('COREPATH', $template_directory . DS . 'external/fuel/core/');
define('APPPATH', $template_directory . DS . 'external/fuel/app/');
define('DOCROOT', realpath(__DIR__.DS.$_SERVER['doc_root']).DS);
define('PKGPATH', realpath(rtrim($_SERVER['package_path'], '/').'/').DS);
...
if (isEnvironment(Fuel::DEVELOPMENT) || isEnvironment(Fuel::TEST) || isEnvironment(Fuel::STAGING)) {
    error_reporting(E_ALL ^ E_DEPRECATED);
    ini_set("display_errors", 1);
    ini_set("log_errors", 0);
} else if(isEnvironment(Fuel::PRODUCTION)) {
    error_reporting(0);
    ini_set("display_errors", 0);
    ini_set("log_errors", 1);
}

if (isEnvironment(Fuel::DEVELOPMENT)) {
...
function setEnvironment() {
    ...
    } else if (defined('STAGING')) {
        if (constant('STAGING') == true) {
            Fuel::$env = (isset($_SERVER['FUEL_ENV']) ? $_SERVER['FUEL_ENV'] : Fuel::STAGING);
            return true;
        }
    } else if (defined('PRODUCTION')) {
        if (constant('PRODUCTION') == true) {
...
function isEnvironment($environment = Fuel::DEVELOPMENT) {
    if (Fuel::$env === $environment) {
        return true;
    }
    return false;
}
